...
| `R2_ALLOWED_CONTENT_TYPES`          | chat attachments          | optional (default `image/png,image/jpeg,image/webp`; PDF deliberately excluded until the KB agent lands — see [docs/ATTACHMENTS.md § Scope today](docs/ATTACHMENTS.md#scope-today-images-only); read by both server validator and composer file-picker) |
