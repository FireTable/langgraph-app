...
- **Daily VPS backup**: a `backup` sidecar in the same `docker-compose.yml` (gated by `profiles: [backup]`) dumps Postgres + tars every deploy secret on a 04:00 UTC cron, then optionally syncs to Google Drive via `rclone` so a VPS loss is recoverable. See [docs/BACKUP.md](docs/BACKUP.md).
...
| `R2_FOLDER_USER`                    | chat attachments + KB ingest + avatars | optional (default `u`; user-scoped R2 root — all chat attachment, KB derived image, and avatar keys sit under this prefix via `lib/r2/keys.ts`; no migration ships, so a value change leaves old objects unreachable)                                                                                              |
...
- [`docs/CANVAS.md`](docs/CANVAS.md) — chat-first → canvas split view: React Flow integration, one-row `canvas_snapshots` schema, debounce + `sendBeacon` auto-save, the `generate_image` LangGraph tool (FAL_KEY gated, `kind: "pic"` credit attribution).
