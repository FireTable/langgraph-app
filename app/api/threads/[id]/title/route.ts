...
// response.
//
// For now we just use the first 8 words of the first user message. A
// future iteration can call the LLM here, but a deterministic placeholder
// is enough for the adapter to wire up.
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  await ctx.params; // acknowledge id (used for routing)
  ...
  const title = (firstUser?.content ?? "New chat").split(/\s+/).slice(0, 8).join(" ");
