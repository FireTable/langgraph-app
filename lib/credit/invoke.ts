...
/**
 * Standard credit-exceeded assistant reply. Same shape the model would
 * produce, so the UI doesn't have to special-case it.
 *
 * Nodes catch `CreditExceededError` from their `chatModel.invoke(...)`
 ...
 */
export function creditExceededReply(err: CreditExceededError): AIMessage {
