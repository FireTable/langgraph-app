...
// ponytail: Execution engine stripped. The agent orchestrates
// generate_image via the LangGraph tool; the canvas node just renders
// the user's input + the agent's last result. No isExecuting /
// isGraphRunning state — the whole play/stop flow is gone. The "Run
// pipeline" trigger lives at the canvas level (the toolbar), not on
// each node.
export class NodeShapeUtil extends ShapeUtil<NodeShape> {
  ...
  override onResize(shape: any, info: any) {
    ...
    if (definition.canResizeNode) {
      const node = shape.props.node as unknown as { w: number; h: number; type: string };
