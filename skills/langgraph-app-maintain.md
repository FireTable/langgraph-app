...
| `LLM_KEY_ENCRYPTION_KEY`                    | `openssl rand -hex 32` (local) — same generator as `BETTER_AUTH_SECRET`        | AES-256-GCM KEK that wraps every `provider.apiKeys[]` entry. Required to start the admin UI — server returns 503 if missing or malformed. **Set once and forget** — rotating requires re-encrypting every row (out of scope, see issue #15).                    | **Required** if using the admin-managed provider key pool (issue #15). Only the first admin needs it set; without it you can still bootstrap with `OPENAI_API_KEY` env as a fallback (legacy path). |
| `INITIAL_ADMIN_EMAIL`                       | Your email address (e.g. `you@example.com`)                                    | On signup, the first user matching this email (case-insensitive) is promoted to `roleId='admin'` via Better Auth's `databaseHooks.user.create.after`. Idempotent — leave set forever; only the FIRST signup is promoted. Add subsequent admins via `/admin` UI. | Optional (without it, the first user who signs up gets `roleId='user'` and must be promoted via SQL or `INITIAL_ADMIN_EMAIL`)                                                                       |
...
#   # Issue #15 — admin-managed provider key pool (AES-256-GCM):
#   LLM_KEY_ENCRYPTION_KEY=$(openssl rand -hex 32)
#   INITIAL_ADMIN_EMAIL=you@example.com        # optional, first admin bootstrap
...
Reset scenarios: changed `BETTER_AUTH_SECRET` / `POSTGRES_PASSWORD` (old sessions are dead), `LLM_KEY_ENCRYPTION_KEY` rotated without re-encryption (admin key pool becomes unreadable — must restore the old KEK), migration deadlocked, debugging.
