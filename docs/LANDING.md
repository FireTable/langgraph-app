...
| Features     | `components/landing/features.tsx`     | 4-card bento (`big` headliner + `wide` Memory + KB + two `default`) over a 3-card row (Composable / HITL / Self-host). Hue palette in `HUE` carries both the icon-chip class and the card-level tint, so each card wears its own colour, not just a dot. |
| How it works | `components/landing/how-it-works.tsx` | Six scroll-driven explainers (Streaming, Multi-graph, Memory, Observability, HITL, Knowledge base). Each motion demo is a separate client island; the surrounding copy is server-rendered so it's in the initial HTML.                                   |
...
- `sections.test.tsx` — pins the four feature headings + the six How-it-works explainer rows (incl. KB/GraphRAG). Refactors that drop a title or rewrite it ambiguously fail here.
