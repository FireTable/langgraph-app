...
| `generate_image`         | `FAL_KEY` (optional)           | Mock-first pattern (NOT rule #10's free-tier exemption — `fetch_url`/`r.jina.ai` is the real free-tier case). Without the key the tool returns a `placehold.co` mock URL so local dev / demos still work end-to-end; production adds the key to hit real fal.ai flux/schnell and the mock branch goes dead. |
