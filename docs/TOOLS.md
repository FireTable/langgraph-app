...
| Tool             | Backend file | Frontend card                                   | Notes                                                                                                                                                                                                                                                                                        |
| ---------------- | ------------ | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `search_kb`      | `kb.ts`      | `components/assistant-ui/kb-search-tool-ui.tsx` | Hybrid RRF (k=60) over BM25 (`tsv` GIN) + pgvector cosine (`embedding` HNSW) + entity-tag overlap (`entities` GIN). Returns structured JSON: `{ content, documents[], empty }`. `content` embeds `[1] [2] [3]` markers; `documents[]` carries `chunkId`/`docId`/`rrfScore`/`legsHit` for UI. |
| `search_graph`   | `kb.ts`      | `components/assistant-ui/kb-search-tool-ui.tsx` | Same machinery, forces `qents = query` so the entity-tag leg dominates. Use for multi-hop / entity-relationship queries.                                                                                                                                                                     |
