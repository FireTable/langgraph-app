...
- **Thread visibility in chat sidebar**: `threads.kind` (`'chat' | 'kb' | 'eval-judge' | 'eval-benchmark'`) is filtered at `lib/threads/queries.ts:listThreadsForUser` — `WHERE kind='chat'` keeps KB threads (and the two `eval-*` kinds) out of the user's chat sidebar. The settings page is the only UI entry into KB observability. The `eval-*` kinds are hidden from the sidebar because they are scoring / synthetic work — they surface in the Admin Eval Studio instead.
