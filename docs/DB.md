...
| `prompt_template`           | app   | System prompt definitions and version history                                                                                                                                                                                                                                     |
| `prompt_variant`            | app   | A/B variants with configurable traffic weights (0-100%)                                                                                                                                                                                                                           |
| `prompt_variant_assignment` | app   | Sticky variant binding per `(user_id, agent)`                                                                                                                                                                                                                                     |
| `eval_run`                  | app   | Turn execution facts with latency, tokens, and `parent_message_id` for JOINing `observability_spans`                                                                                                                                                                              |
| `eval_feedback`             | app   | User online ratings (1-5, 👍=5, 👎=1) and admin ratings                                                                                                                                                                                                                           |
| `eval_rubric`               | app   | Per-agent evaluation criteria definitions for LLM-as-a-Judge (keyed directly by `id = "rubric_${agentId}"`)                                                                                                                                                                       |
| `eval_benchmark`            | app   | Offline benchmark test cases per agent (`id`, `agent`, `title`, `input_prompt`, `expected_output`)                                                                                                                                                                                |
| `eval_judgment`             | app   | Assessment score and reasoning outputs emitted by `evalAgent`, with `judge_thread_id` tracking AI Judge execution traces                                                                                                                                                          |
...
| `kind`            | text         | `"chat"` \| `"kb"` \| `"eval-judge"` \| `"eval-benchmark"`; `DEFAULT 'chat'`. The two `eval-*` kinds are hidden from both the chat sidebar and Online Executions (benchmark) / are scoped to their evaluation surface (judge) — see `docs/EVALUATION.md`. |
...
| `prompt_template`           | `app/api/eval/prompts` (GET), `backend/prompt/loader.ts` (`getAgentPrompt`)                                                          | `app/api/eval/prompts` POST (`create_template` / PUT for content edit); `lib/eval/queries.ts:seedInitialPrompts` (idempotent)                                                                                                                     |
| `prompt_variant`            | `app/api/eval/prompts` (GET), `app/api/eval/assignments` (cohort join), `backend/prompt/loader.ts` (`assignPromptVariant`)           | `app/api/eval/prompts` POST (`create_variant` / `update_variant_weight` / `batch_update_weights` / `create_cohort_variant` / `delete_cohort_variant`); `lib/eval/queries.ts:seedInitialPrompts`                                                   |
| `prompt_variant_assignment` | `app/api/eval/assignments` (GET), `backend/node/prepare-data-node.ts` (sticky lookup)                                                | `app/api/eval/assignments` POST (cohort / single agent reassignment); `prepareDataNode` first-touch insert                                                                                                                                        |
| `eval_run`                  | `lib/eval/queries.ts:getRunsByAgentPage`, `app/api/eval/runs/{compare,page,[id]}`                                                    | `lib/eval/callback.ts` (`EvalCallbackHandler` writes on every chat-model `handleLLMEnd`); `lib/eval/queries.ts:recordEvalRun` (called by `evalAgent` benchmark mode)                                                                              |
| `eval_feedback`             | `app/api/eval/runs/[id]` (single-run detail), `lib/eval/queries.ts:getRunsByAgentPage` (LEFT JOIN for ratings)                       | `app/api/eval/feedback` POST — `lib/eval/queries.ts:submitFeedback` resolves `runId` against `eval_run.id` or `parent_message_id`; silently no-ops when no match (chat thumbs-up never FK-violates)                                               |
| `eval_rubric`               | `app/api/eval/rubrics` (GET), `backend/agent/eval-agent.ts:judgeByLLMNode` (`rubric_${agent}` lookup), `app/api/eval/judge/route.ts` | `app/api/eval/rubrics` POST; `app/api/eval/benchmarks` POST `update_rubric` action; `lib/eval/queries.ts:seedInitialPrompts` seeds `rubric_default` + one rubric per agent (incl. `judgeByLLM`)                                                   |
| `eval_benchmark`            | `app/api/eval/benchmarks` (GET), `lib/eval/queries.ts:getAllBenchmarksWithLatestJudgment` (LEFT JOIN on `latest_judgment_id`)        | `app/api/eval/benchmarks` POST `create` / `delete`; `lib/eval/queries.ts:seedInitialPrompts` seeds one benchmark per agent; `backend/agent/eval-agent.ts:judgeByLLMNode` denormalizes `latest_judgment_id` / `latest_score` after every judge run |
| `eval_judgment`             | `app/api/eval/rubrics` (GET, joined to `eval_run`); `app/api/eval/runs/[id]` (single-run detail)                                     | `backend/agent/eval-agent.ts:judgeByLLMNode` (`saveJudgment`)                                                                                                                                                                                     |
