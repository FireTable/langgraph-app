...
|               |                                                                                                                                                                      |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Query params  | `?deleteAll=true` → cascade-delete every doc in the folder (each doc's chunks, embeddings, attachments) before removing the folder. Omit for the empty-only default. |
| Request body  | (none)                                                                                                                                                               |
| 204 response  | (empty body)                                                                                                                                                         |
| Failure codes | 401 `UNAUTHORIZED`. 404 `NOT_FOUND`. 409 `NON_EMPTY` (`{ docCount }`) — only when `?deleteAll=true` is **not** set.                                                  |
...
| 200 response  | default: `{ groups: Array<{ folder: { id, name, docCount }, documents: Array<{ id, title, status, errorMessage, contentType, attachmentId, attachmentUrl, createdAt, updatedAt, totalPages, failedPages, pendingPages, parsingPages, totalChunks, successChunks, embeddingPendingChunks, failedChunks, pendingChunks, parsingChunks, entityCount, relationshipCount }> }> }` <br/>`?mention=1`: `{ folders: Array<{ id, name, docCount, docs: Array<{ id, title, status }> }> }` |
| Failure codes | 401 `UNAUTHORIZED`. 500 `INTERNAL`.                                                                                                                                                                                                                                                                                                                                                                                                                                              |

> **Indexing counts in the doc payload** — `successChunks` counts chunks with `status='success' AND embedding IS NOT NULL`. `embeddingPendingChunks` is chunks with `status='success' AND embedding IS NULL` (OCR done, vector pending). `entityCount` / `relationshipCount` are graph rows for the doc (hybrid-search third leg). The Settings → KB "Indexed" badge requires `successChunks === totalChunks AND (entityCount > 0 OR relationshipCount > 0)`; the chat-side `ChunksStatusBadge` enforces the same gate.
...
> **Internal change, same contract.** Post issue #45 (audit §8 / migrations `0012_blue_steve_rogers.sql`, `0013_lowly_whirlwind.sql`, `0014_bright_norrin_radd.sql`), per-chunk `entities` / `themes` are no longer stored on `kb_chunk`. `entities` are JOINed server-side from `kb_entity` via `source_chunk_ids && ARRAY[chunkId]`; `themes` are JOINed from the flat `kb_theme` table on `chunk_id`; `relationships` from `kb_relationship`. Wire shape (`chunks[i].entities/relationships/themes`) is unchanged so the Settings → KB doc-detail UI reads them transparently.

|               |                                                                                                                                                                                                                                                                                                                                                                                |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 200 response  | `{ doc: { id, title, status, errorMessage, contentType, attachmentId, folderId, contentHash, pages, createdAt, updatedAt, entityCount, relationshipCount }, chunks: Array<{ ordinal: number, content: string, status, errorMessage, hasEmbedding, entities: Array<{name,type,description}>, relationships: Array<{source,target,relation,description}>, themes: string[] }> }` |
| Failure codes | 401 `UNAUTHORIZED`. 404 `NOT_FOUND`.                                                                                                                                                                                                                                                                                                                                           |

> **`hasEmbedding`** — `true` iff `kb_chunk.embedding IS NOT NULL` (pgvector vector stored). A chunk with `status='success'` and `hasEmbedding=false` is in the "Embedding" intermediate state — OCR text was written but the vector hasn't been produced yet. The doc-level "Indexed" badge ignores chunks where `hasEmbedding=false` for its gate.
...
| `?mode=retryFailedChunks` | In-place UPDATE failed chunks to `status='parsing'` (id / ordinal / embedding / content preserved); keep doc row + good chunks; re-run chunk-extract on the failed chunks only | **stays `success`**              | doc `status !== 'success'` OR no failed chunks |
...
|               |                                                                                                                                                                                                                                                                                |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Request body  | `{ id?: string (^[a-z0-9_-]+$, 1..64; defaults to a slug derived from `name`), name: string (1..128), baseUrl: url (REQUIRED — OpenAI-compatible endpoint), enabled?: boolean (default true), apiKeys?: ProviderApiKey[] (default []), models?: ModelConfig[] (default []) }`. |
| 201 response  | `PublicProvider`                                                                                                                                                                                                                                                               |
| Failure codes | 400 `BAD_REQUEST`, 401, 403, 409 `DUPLICATE` (PK collision on the derived or supplied `id`).                                                                                                                                                                                   |
...
|               |                                                                                                                                                                                             |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Request body  | `{ name: string (1..128), enabled: boolean, inputPer1k: number (≥0), outputPer1k: number (≥0), kind?: ("chat"\|"ocr"\|"embed"\|"extract"\|"rerank"\|"eval")[] (min 1, default ["chat"]) }`. |
| 201 response  | `PublicProvider`                                                                                                                                                                            |
| Failure codes | 400 `BAD_REQUEST`, 401, 403, 404 `NOT_FOUND` (provider missing), 409 `DUPLICATE_MODEL`.                                                                                                     |

`kind` is the set of pools this model serves — `chat` for general inference, `ocr` for vision-based PDF text extraction, `embed` for KB chunk vectors, `extract` for structured-output extraction, `rerank` for document reranking, and `eval` for LLM-as-a-Judge. A single upstream model can hold multiple (e.g. `["chat","ocr"]`). When omitted, the backend writes `["chat"]`. See [`docs/PROVIDERS.md`](./PROVIDERS.md) for how the registry routes traffic across `(provider, model, key, kind)` tuples.
...
|               |                                                                                                                                                                               |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Request body  | Any subset of `{ name?, enabled?, inputPer1k?, outputPer1k?, kind?: ("chat"\|"ocr"\|"embed"\|"extract"\|"rerank"\|"eval")[] (min 1) }`. Empty body returns 400 `BAD_REQUEST`. |
| 200 response  | `PublicProvider`                                                                                                                                                              |
...
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Request body  | (none)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 200 response  | `{ users: { id, name, email, image, emailVerified, roleId, roleName, banned, createdAt, updatedAt, todayCredits, todayInputTokens, todayOutputTokens, todayTokens, totalCredits, totalInputTokens, totalOutputTokens, totalTokens }[] }`. `roleName` is `null` if the FK target is missing (defensive — FK prevents it on a healthy DB; a leftJoin can still null it on race). The `today*` fields scope to UTC-midnight; the `total*` fields are lifetime. All token counts come from `credit_usage_log` rows where `status='success'`. |
| Failure codes | 401, 403.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Request body  | (none)                                                                                                                                                                                                                                                                                                                                                                                                            |
| 200 response  | `{ used, limit \| null, windowHours \| null, resetAt, unlimited, roleName, todayCredits, todayInputTokens, todayOutputTokens, todayTokens, totalCredits, totalInputTokens, totalOutputTokens, totalTokens }` — `limit` and `windowHours` are `null` when `unlimited`. The `today*` fields scope to UTC-midnight; the `total*` fields are lifetime. Sourced from `credit_usage_log` rows where `status='success'`. |
