...
| VPS daily backup (sidecar cron + rclone to Drive) | `docs/BACKUP.md`         |
...
- **Backend graph**: two compiled graphs — `agent` (`backend/agent.ts`) and `backgroundAgent` (`backend/background-agent.ts`). Adding a sub-agent: update the enum in `state.ts` AND `router-agent-node.ts` AND `agent.ts` routeToSubAgent union (or parsing throws — see [[router-decision-schema-duplicated]]).
